...
local function scoreTrailhead(m, row, col)
...
local function rateTrailhead(m, row, col)
    assert(m[row][col] == 0, "Starting position is not 0")
    local ROWS, COLS = #m, #m[1]
    local stack = {{row, col, 0}}
    local dirs = {{1, 0}, {0, 1}, {-1, 0}, {0, -1}}
...
local function scoreTrailheads(m)
    ...
    for i, _ in pairs(m) do
        for j, _ in pairs(m[i]) do
            if m[i][j] == 0 then sum = sum + scoreTrailhead(m, i, j) end
        end
    end
    return sum
end

local function rateTrailheads(m)
    local sum = 0
    for i, _ in pairs(m) do
        for j, _ in pairs(m[i]) do
            if m[i][j] == 0 then sum = sum + rateTrailhead(m, i, j) end
        end
    end
    return sum
end
